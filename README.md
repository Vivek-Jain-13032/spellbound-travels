...
- This is intentionally a single scrolling page with no Angular Router — "Home / Services / Contact" are anchor sections (`#home`, `#services`, `#contact`), not separate routes.
- **The About section is temporarily commented out** (not deleted) in `src/app/app.component.ts`/`.html`, and the lead-capture form (`#contact`) was moved to sit right after the hero instead of at the bottom of the page, so it's the first thing a visitor can interact with rather than the last section after several scrolls. The "About" nav link is commented out to match, in both `navbar.component.ts` and `footer.component.ts` (each keeps its own copy of the link list). To restore the original layout: uncomment `<app-about>`/the `AboutComponent` import in `app.component.ts`, uncomment the three `{ id: 'about', label: 'About' }` entries, and move the `#contact` `<section>` back below `<app-why-choose-us>` in `app.component.html`.
- **The navbar/footer "Contact" link now scrolls to the footer** (`id="footer"` on the `<footer>` element), not the lead-form section — the form itself kept its `id="contact"` unchanged, since the "Plan Your Journey" CTA, the hero/services "Enquire Now" buttons, and `NavigationService.enquire()`'s default target all still need to land on the form specifically, not the footer. Only the two nav-link lists' "Contact" entry was retargeted. One consequence: `ScrollSpyDirective` (`src/app/directives/scroll-spy.directive.ts`) originally only used an `IntersectionObserver` with a narrow centered "band" to decide which nav link to highlight — a section shorter than the viewport (like the footer, now the last thing on the page) can never cross that band once scrolling hits the document's bottom limit, so it never got highlighted. The directive now also falls back to a plain `scroll` listener checking "are we scrolled to the bottom of the page, and is this the element that's actually last in the document" (via `offsetTop`/`offsetHeight`, not `getBoundingClientRect`, since the latter reflects the reveal-animation's CSS transform mid-transition and would flicker).
...
- The footer email link opens Gmail's web compose UI directly (`footer.component.ts`'s `gmailComposeHref`, `https://mail.google.com/mail/?view=cm&fs=1&to=...`) in a new tab, rather than a plain `mailto:` link — a `mailto:` defers to whatever desktop mail client is registered as the OS default (which may not be Gmail), so this pins it to Gmail specifically instead.
- `public/privacy-policy.html` is a standalone static page, not an Angular route or component — the site is intentionally router-free (see above), and a single static legal page doesn't justify adding one. It's plain self-contained HTML/CSS (no Tailwind build, no Angular), styled to match the site by hand, linked from the footer's copyright row. It documents what the Site actually does per the code: the lead form's fields, Google Tag Manager's event tracking (button clicks / form-funnel progress / lead submission — deliberately configured to send the service type and preferred contact method, not the name/email/phone/message you type in), the Google Translate language cookie, and EmailJS/WhatsApp as the third-party processors. **This is a drafted starting point, not a substitute for legal review** — have it checked against applicable law (e.g. India's DPDP Act, GDPR if you serve EU visitors) before relying on it, and keep it in sync if the form fields, analytics setup, or third-party services change.
