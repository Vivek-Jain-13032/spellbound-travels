...
3. Available template variables (populated from the form): `to_admin_email`, `to_email`, `full_name`, `email`, `phone`, `service`, `flying_from`, `flying_to`, `departure_date`, `return_date`, `passengers`, `travel_class`, `nationality`, `destination_country`, `visa_type`, `expected_travel_date`, `message`.
